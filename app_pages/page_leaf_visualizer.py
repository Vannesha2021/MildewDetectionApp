...
def page_leaf_visualizer_body():
    ...
    if st.checkbox("Difference between average and variability image"):
        avg_powdery_mildew = plt.imread(f"outputs/v1/avg_var_powdery_mildew.png")
        ...
        st.warning(
            ...
            f"However, a small difference in color pigment of the average images"
            " is seen for both labels")

        st.image(avg_powdery_mildew, caption='Mildew Leaves - Average and Variability')
        st.image(avg_healthy, caption='Healthy leaves - Average and Variability')
        st.write("---")

    if st.checkbox("Differences between average powdered mildew and healthy leaves"):
